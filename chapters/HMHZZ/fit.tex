...
The likelihood function can be written as the product of the probability of observing $n$ events in Poisson distribution 
and the sum of signal and background weighted following their probability density functions (PDFs):
\begingroup
\small
...
The parameters of interest (POI) in this search is $\sigma_{\textrm{ggF}}$ (and $\sigma_{\textrm{VBF}}$ only for NWA signal), 
presenting the cross section of signal in ggF (and VBF) production mode.
In the case of two POIs, when testing one POI, the other is left unconstrainted and profiled with other nuisance parameters during the minimization. 
The expected signal yields $S_{\mathrm{ggF}}$ and $S_{\mathrm{VBF}}$ in equation~\ref{eq:likelihood} can be written from these POIs:
\begingroup
\small
\begin{equation}
...
\end{equation}
\endgroup
where $B(S\rightarrow ZZ)$ is the branching ratio of signal decay to two Z bosons,
$A\times C$ is the parameterized signal acceptance as described in~\ref{sec:hmhzz_signal_acc}, 
and $\int\mathcal{L} = \lumi $ is the integrated luminosity of data.

The systematic uncertainties of the expected number of signal and background events (normalizations) and the shape of the PDFs measured in section~\ref{sec:hmhzz_sys} 
is described by a set of nuisance parameters (NPs) $\theta_{i}$, constrained by Gaussian distribution described in section~\ref{sec:NPs} in fit.
Thus, the final likelihood function ${\cal L } ( \sigma_{\mathrm{ggF}}, \sigma_{\mathrm{VBF}},\mH,\theta_{i})$ is modelled as a function of several variables: $\sigma_{\mathrm{ggF}}, \sigma_{\mathrm{VBF}} , \mH$, and $\theta_{i}$.
...
Given that the increased data luminosity can now provide precise determination of the SM $ZZ$ background with only about 2\% statistical uncertainty,
floating $ZZ$ normalization in fit reduces the dependence on theory predictions and their corresponding large uncertainties (about 30\%).

%%% comparison: fixing vs floating muZZ
In addition, the study on comparison of expected signal sensitivity between fixing (with Gaussian constraints on its normalization uncertainties) 
and floating $ZZ$ normalization has been done.
Table~\ref{tab:muZZ_limits} shows the value of expected 95\% $CLs$ upper limits under these two cases at three different mass points.
\begin{table}[htbp]
  \centering
  \caption{Summary of $CLs$ upper limits on ggF production mode at 300, 600, 1000~\gev.}
  \label{tab:muZZ_limits}
  \begin{tabular}{c|c|c|c}
    \hline
    $\sigma_{\mathrm{ggF}} \times B(S\rightarrow ZZ)$ [pb] & 300 ~\gev  & 600 ~\gev & 1000 ~\gev  \\
    \hline
    Floating $ZZ$ normalization & 0.0892 & 0.0253 & 0.0125 \\
    Fixing $ZZ$ normalization   & 0.0892 & 0.0254 & 0.0125 \\
    \hline
  \end{tabular}
\end{table}
The results prove that with the excellent discrimination power of \mfl distribution between the SM background and the resonant signal, 
floating $ZZ$ normalization in fit would not reduce the expected signal sensitivity.
%%%


At the end, the upper limit on production cross-section $\sigma_{\mathrm{ggF(VBF)}}$ at a given heavy resonance model is obtained by fixing the mass of signal ($m_{H}$) at the desired value,
and maximising the likelihood function.
...
%In order to measure the compatibility of the data with the background-only hypothesis ($\sigma_{\mathrm{ggF}}=\sigma_{\mathrm{VBF}}=0$), the test statistic $q_{0}$ is used as introduced in section~\ref{sec:teststat}.
...
\begin{table}[tbp]
    \caption{\label{tab:postfit_4lyields}
      ...
      for three \textbf{MVA-based} categories.
    ...
    \resizebox{\textwidth}{!}{
      \begin{tabular}{l r@{ $\pm$ }l r@{ $\pm$ }l r@{ $\pm$ }l r@{ $\pm$ }l r@{ $\pm$ }l }
        \toprule
        \multirow{2}{*}{Process} & \multicolumn{2}{c}{VBF-MVA-enriched} & \multicolumn{6}{c}{ggF-MVA-high}  & \multicolumn{2}{c}{ggF-MVA-low} \\
...
\begin{table}[tbp]
    \caption{\label{tab:postfit_4lyields_cut}
      Expected and observed numbers of events for $\mfl > 200~\gev{}$, 
      together with their systematic uncertainties, 
      for two \textbf{Cut-based} categories.
      The expected number of events, as well as their uncertainties, are obtained
      from a likelihood fit to the data under the background-only hypothesis.
      The uncertainties of the $ZZ$ normalisation factors, presented in table~\ref{tab:muZZ_bonly_cut},
      are also taken into account.
    }
    \centering
    ...
    \resizebox{\textwidth}{!}{
      \begin{tabular}{l r@{ $\pm$ }l r@{ $\pm$ }l r@{ $\pm$ }l r@{ $\pm$ }l r@{ $\pm$ }l }
        \toprule
        \multirow{2}{*}{Process} & \multicolumn{2}{c}{VBF-CBA-enriched} & \multicolumn{6}{c}{ggF-CBA-enriched} \\
        & \multicolumn{2}{c}{} & \multicolumn{2}{c}{$4\mu$ channel} & \multicolumn{2}{c}{$2e2\mu$ channel} & \multicolumn{2}{c}{$4e$ channel} \\ 
        \midrule
  
        \qqZZ	        &	48	&	8	&	860	&	18	&1360	&	28	&	515	&	11	\\
        \ggZZ	        &	13	&	4	&	114	&	9	&	189	&	14	&	73	&	6	\\
        \ZZ (EW)    &	10.9	&	0.9	&	6.9	&	0.3	&11.1	&	0.4	&	4.4	&	0.2	\\
        \Zjet,\ttbar	&	0.3	&	0.1	&	2.1	&	0.4	&	6.7	&	1.6	&	3.1	&	0.4	\\
        \ttv,\vvv	    &	3.0	&	0.2	&16.3	&	0.4	&26.8	&	0.6	&11.8	&	0.3	\\
        \midrule
        Total background	&	75	&	9	&	1000	&	18	&	1593	&	28	&	607	&	11	\\
        
        \midrule
        Observed  & \multicolumn{2}{c}{75} & \multicolumn{2}{c}{932} & \multicolumn{2}{c}{1656} & \multicolumn{2}{c}{612} \\
        \bottomrule
      \end{tabular}
    }
\end{table}
...
Then due to the same background estimation and modelling procedures, as well as the same method of systematic measurements between MVA- and Cut- based analysis,
the following results of background-only fits in this selection are only shown in MVA-based analysis under the model of heavy Higgs resonance with narrow-width as an example.
The final results of interpretation in both MVA- and cut- based analysis in different signal models described in section~\ref{sec:signal_model} will be presented in next section.
